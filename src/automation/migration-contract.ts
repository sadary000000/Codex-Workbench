...
export { AUTOMATION_ID_FIELDS, assertLegacyMigrationMapping, assertMigrationDocumentEquivalent, assertMigrationIdentityPreserved, migrationIdentityFingerprint, type LegacyMigrationDelta } from "./migration-identity.ts";
import { assertLegacyMigrationMapping, assertMigrationIdentityPreserved, type LegacyMigrationDelta } from "./migration-identity.ts";
...
export interface MigrationInspection {
  ...
  readonly legacyMigrationDelta: LegacyMigrationDelta | null;
}
...
function invalidInspection(status: "UNSUPPORTED" | "CORRUPT", sourceFormat: MigrationInspection["sourceFormat"], version: number | null, message: string): MigrationInspection {
  return { status, sourceFormat, sourceSchemaVersion: version, targetSchemaVersion: AUTOMATION_SCHEMA_VERSION, document: null, message, legacyMigrationDelta: null };
}
...
export function inspectAutomationMigration(value: unknown, sourceFormat: MigrationInspection["sourceFormat"] = "json"): MigrationInspection {
  ...
  try {
    ...
    const legacyMigrationDelta = migrated.migratedFrom === null ? null : assertLegacyMigrationMapping(value, migrated.document, migrated.migratedFrom);
    return {
...
export class AutomationMigrationService {
  ...
  async inspect(): Promise<MigrationInspection> {
    ...
    if (file.kind === "missing") return { status: "READ_COMPATIBLE", sourceFormat: "missing", sourceSchemaVersion: null, targetSchemaVersion: AUTOMATION_SCHEMA_VERSION, document: null, message: null, legacyMigrationDelta: null };
    if (file.kind === "json") {
    ...
    if (file.kind === "sqlite") {
      ...
      if (inspection.status === "needs_migration") return { status: "MIGRATION_REQUIRED", sourceFormat: "sqlite", sourceSchemaVersion: inspection.migratedFrom, targetSchemaVersion: AUTOMATION_SCHEMA_VERSION, document: null, message: inspection.message, legacyMigrationDelta: null };
      ...
      return { status: "READ_COMPATIBLE", sourceFormat: "sqlite", sourceSchemaVersion: AUTOMATION_SCHEMA_VERSION, targetSchemaVersion: AUTOMATION_SCHEMA_VERSION, document: inspection.document, message: null, legacyMigrationDelta: null };
    }
