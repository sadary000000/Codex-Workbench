...
import { assertLegacyMigrationMapping, assertMigrationDocumentEquivalent, type LegacyMigrationDelta } from "./migration-identity.ts";
...
export interface AutomationPersistenceDiagnostics {
  ...
  migration: {
    ...
    legacyMigrationDelta: LegacyMigrationDelta | null;
  };
}
...
function assertRowIdentity(row: SqliteRow, table: AutomationTableName, item: unknown): void {
  const expectedEntityId = idFor(table, item);
  if (row.entity_id !== expectedEntityId) {
    throw new AutomationPersistenceError("AUTOMATION_DB_INVALID", `SQLite row identity does not match ${table}.${ID_FIELDS[table]}.`);
  }
  const expectedProjectId = projectIdFor(table, item);
  if (row.project_id !== expectedProjectId) {
    throw new AutomationPersistenceError("AUTOMATION_DB_INVALID", `SQLite row project scope does not match ${table}.${ID_FIELDS[table]}.`);
  }
}
...
function readDocumentRows(database: DatabaseSync, validateDocument = true): AutomationDocument {
  ...
  for (const row of rows) {
    ...
    assertRowIdentity(row, row.table_name, item);
...
export class SqliteAutomationPersistence {
  ...
  diagnostics(): AutomationPersistenceDiagnostics {
    ...
    return {
      ...
      migration: {
        ...
        legacyMigrationDelta: this.jsonMeta<LegacyMigrationDelta>("migration_legacy_delta"),
      },
    };
  }
  ...
  private initializeMetadata(migration?: AutomationMigrationMetadata): void {
    ...
    if (existingDocumentVersion === "2" || existingDocumentVersion === "3") {
      ...
      const migratedResult = migrateAutomationDocument(legacy);
      const migrated = migratedResult.document;
      const legacyMigrationDelta = assertLegacyMigrationMapping(legacy, migrated, Number(existingDocumentVersion));
      try {
        this.database.exec("BEGIN IMMEDIATE");
        const upsert = this.database.prepare(`
          INSERT INTO automation_records (table_name, entity_id, project_id, payload)
        ...
        if (effectiveMigration) {
          ...
          this.setMeta("migration_legacy_delta", JSON.stringify(effectiveMigration.legacyMigrationDelta ?? legacyMigrationDelta));
        }
    ...
    if (effectiveMigration) {
      ...
      this.setMeta("migration_source_backup_path", effectiveMigration.sourceBackupPath);
      this.setMeta("migration_at", effectiveMigration.migratedAt);
      if (effectiveMigration.legacyMigrationDelta) this.setMeta("migration_legacy_delta", JSON.stringify(effectiveMigration.legacyMigrationDelta));
    }
  }
  ...
  private jsonMeta<T>(key: string): T | null {
    const value = this.meta(key);
...
export async function migrateJsonSnapshotToSqlite(filePath: string, raw: string): Promise<SqliteAutomationPersistence> {
  ...
  let legacyMigrationDelta: LegacyMigrationDelta | null = null;
  let parsed: unknown;
  try {
  ...
  try {
    ...
    legacyMigrationDelta = migrated.migratedFrom === null ? null : assertLegacyMigrationMapping(parsed, migrated.document, migrated.migratedFrom);
  } catch (error) {
  ...
  try {
    ...
    if (sourceBackedUp) {
      const currentKind = await fileKind(filePath);
      if (!promoted && currentKind === "missing") {
        await rename(backup, filePath).catch(() => undefined);
      } else if (promoted) {
        // Promotion happened, but reopening the promoted file failed. Restore
        // the original source instead of leaving a partial/corrupt candidate
        // at the canonical path.
        if (currentKind !== "missing") await rm(filePath, { force: true }).catch(() => undefined);
        await rename(backup, filePath).catch(() => undefined);
      }
    }
