...
const PROVIDER_REF_KIND = "OTHER" as const;
const PROVIDER_REF_PREFIX = "automation-provider-ref-v1:";
const LEGACY_REQUEST_KIND = "WEBGPT_PROVIDER_REQUEST" as const;
const LEGACY_OBSERVATION_KIND = "WEBGPT_PROVIDER_OBSERVATION" as const;

type ProviderRefRole = "REQUEST" | "OBSERVATION";
...
function encodeProviderOpaqueId(role: ProviderRefRole, providerOpaqueId: string): string {
  const normalized = providerOpaqueId.trim();
  if (!normalized || normalized.length > 384 || /[\r\n]/.test(normalized)) {
    throw new AutomationStoreError("AUTOMATION_INVALID", "Provider opaque identity must be bounded non-empty text.");
  }
  const encoded = `${PROVIDER_REF_PREFIX}${role.toLowerCase()}:${encodeURIComponent(normalized)}`;
  if (encoded.length > 512) throw new AutomationStoreError("AUTOMATION_INVALID", "Provider opaque identity exceeds the ExternalRef boundary.");
  return encoded;
}

function decodeProviderOpaqueId(value: string): { role: ProviderRefRole; providerOpaqueId: string } | null {
  if (!value.startsWith(PROVIDER_REF_PREFIX)) return null;
  const body = value.slice(PROVIDER_REF_PREFIX.length);
  const separator = body.indexOf(":");
  if (separator <= 0) return null;
  const roleText = body.slice(0, separator);
  const role: ProviderRefRole | null = roleText === "request" ? "REQUEST" : roleText === "observation" ? "OBSERVATION" : null;
  if (!role) return null;
  try {
...
function refRole(ref: ExternalRef): ProviderRefRole | null {
  if (ref.kind === LEGACY_REQUEST_KIND) return "REQUEST";
  if (ref.kind === LEGACY_OBSERVATION_KIND) return "OBSERVATION";
  if (ref.kind !== PROVIDER_REF_KIND) return null;
  return decodeProviderOpaqueId(ref.opaqueId)?.role ?? null;
}

function providerOpaqueId(ref: ExternalRef): string | null {
  if (ref.kind === LEGACY_REQUEST_KIND || ref.kind === LEGACY_OBSERVATION_KIND) return ref.opaqueId;
  if (ref.kind !== PROVIDER_REF_KIND) return null;
  return decodeProviderOpaqueId(ref.opaqueId)?.providerOpaqueId ?? null;
}

function compatibleRef(ref: ExternalRef, input: { projectId: string; provider: string; role: ProviderRefRole; providerOpaqueId: string }): boolean {
  return ref.projectId === input.projectId
    && ref.provider === input.provider
    && refRole(ref) === input.role
    && providerOpaqueId(ref) === input.providerOpaqueId;
}
...
function assertProviderRefs(intent: ActionIntent, attempt: ActionAttempt, requestRef: ExternalRef | null, observationRef: ExternalRef | null): void {
  if (requestRef) {
    if (requestRef.projectId !== intent.projectId || refRole(requestRef) !== "REQUEST") {
  ...
  if (observationRef) {
    if (observationRef.projectId !== intent.projectId || refRole(observationRef) !== "OBSERVATION") {
  ...
  if (requestRef && observationRef) {
    ...
    const requestOpaque = providerOpaqueId(requestRef);
    const observationOpaque = providerOpaqueId(observationRef);
...
function neutralExternalRef(input: {
  projectId: string;
  provider: string;
  role: ProviderRefRole;
  providerOpaqueId: string;
}): ExternalRef {
  return {
    externalRefId: randomUUID(),
    projectId: input.projectId,
    kind: PROVIDER_REF_KIND,
    provider: optionalText(input.provider, "externalRef.provider", 256)!,
    opaqueId: encodeProviderOpaqueId(input.role, input.providerOpaqueId),
    createdAt: now(),
  };
}

function legacyProjection(ref: ExternalRef): ExternalRef {
  const decoded = ref.kind === PROVIDER_REF_KIND ? decodeProviderOpaqueId(ref.opaqueId) : null;
  if (!decoded) return ref;
  return {
    ...ref,
    kind: decoded.role === "REQUEST" ? LEGACY_REQUEST_KIND : LEGACY_OBSERVATION_KIND,
    opaqueId: decoded.providerOpaqueId,
  };
}

/**
 * ARCH-R2 compatibility store.
 *
 * New provider request/observation identities are persisted through the
 * provider-neutral ExternalRef extension envelope (kind OTHER + a versioned
 * opaque-id envelope + the real provider id). Existing v4 WEBGPT_PROVIDER_*
 * rows remain readable and are never rewritten merely by reading the DB.
 *
 * snapshot() intentionally exposes a legacy compatibility projection while
 * R2 consumers are migrated. snapshotProviderTruth() exposes the actual
 * provider-neutral persisted representation and must be used by new R2 code.
 */
export class ProviderNeutralAutomationStore extends AutomationStore {
  ...
  override async snapshot(): Promise<AutomationDocument> {
    ...
    return {
      ...document,
      externalRefs: document.externalRefs.map((ref) => legacyProjection(ref)),
    };
  }
  ...
  override async persistActionAttemptProviderRequest(input: {
    ...
    return this.transaction((tx) => {
      ...
      const externalRef = existing ?? neutralExternalRef({ projectId: input.projectId, provider: input.provider, role: "REQUEST", providerOpaqueId: providerRequestRef });
  ...
  override async persistActionAttemptProviderObservation(input: {
    ...
    return this.transaction((tx) => {
      ...
      const externalRef = existing ?? neutralExternalRef({ projectId: input.projectId, provider: input.provider, role: "OBSERVATION", providerOpaqueId: providerObservationRef });
      if (!existing) tx.insert("externalRefs", externalRef);
  ...
  override async recordAcceptedProviderUnknown(input: {
    ...
    return this.transaction((tx) => {
      ...
      if (existingReceipt) {
        ...
        if (!existingExternal || refRole(existingExternal) !== "REQUEST") throw new AutomationStoreError("AUTOMATION_CONFLICT", "Existing ActionReceipt lacks its accepted provider request reference.");
      ...
      const externalRef = existingExternal ?? neutralExternalRef({ projectId: input.projectId, provider: input.provider, role: "REQUEST", providerOpaqueId: rawRequestRef });
  ...
  override async createActionReceipt(input: ActionReceiptInput): Promise<ActionReceipt> {
    return this.transaction((tx) => {
      ...
      const nextIntentState = input.status === "SUCCEEDED" ? "COMPLETED" : input.status === "FAILED" ? "FAILED" : "UNCERTAIN";
...
export function isProviderNeutralExternalRef(ref: ExternalRef): boolean {
  return ref.kind === PROVIDER_REF_KIND && decodeProviderOpaqueId(ref.opaqueId) !== null;
}

export function readProviderNeutralExternalRef(ref: ExternalRef): { role: ProviderRefRole; provider: string; providerOpaqueId: string } | null {
  if (ref.kind !== PROVIDER_REF_KIND) return null;
  const decoded = decodeProviderOpaqueId(ref.opaqueId);
  return decoded ? { ...decoded, provider: ref.provider } : null;
}
