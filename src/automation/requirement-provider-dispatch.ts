    ...
    if (accepted.provider !== this.provider.provider || accepted.providerTargetRef !== input.providerTargetRef) {
      try {
        await this.store.recordAcceptedProviderUnknown({
          ...
          externalStatus: "ACCEPTED_IDENTITY_MISMATCH",
        });
      } catch (recoveryError) {
        ...
        throw new RequirementProviderDispatchError("REQUIREMENT_PROVIDER_RECOVERY_REQUIRED", `Provider acceptance identity mismatched and recovery marker persistence failed: ${boundedErrorMessage(recoveryError)}`, recoveryError);
      }
      throw new RequirementProviderDispatchError("REQUIREMENT_PROVIDER_RECOVERY_REQUIRED", "Provider acceptance did not preserve target identity; the accepted side effect is recovery-only and must not be resent.");
    }
    ...
    try {
      ...
      try {
        await this.store.recordAcceptedProviderUnknown({
          ...
          providerRequestRef: accepted.providerRequestRef,
          providerSemanticSha256: accepted.semanticRef ?? null,
          externalStatus: "ACCEPTED_LOCAL_PERSISTENCE_UNCERTAIN",
        });
      } catch (recoveryError) {
        await this.store.transitionActionAttempt(attempt.actionAttemptId, "UNCERTAIN", { actorType: "AUTOMATION", correlationId: intent.intentId }).catch(() => undefined);
        throw new RequirementProviderDispatchError("REQUIREMENT_PROVIDER_RECOVERY_REQUIRED", `Provider accepted the request and recovery marker persistence also failed: ${boundedErrorMessage(recoveryError)}`, recoveryError);
      }
