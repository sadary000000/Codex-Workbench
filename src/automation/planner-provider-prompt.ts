...
export function buildPlannerProviderPrompt(input: PlannerProviderPromptInput): string {
  ...
  return [
    ...
    "If verificationClass is present, it must be exactly one of BUILD, TEST, GIT_DIFF, GIT_STATUS, FILE_EXISTS, HASH_MATCH, JSON_SCHEMA, CLI_SMOKE, HARDWARE_SMOKE, or CUSTOM_APPROVED. Never invent, translate, or emit any other verificationClass value.",
    ...
    "expectedArtifacts, when present, MUST be a JSON array containing only non-empty strings. Never emit expectedArtifacts as a single string or object.",
    "Verifier policy shape example: {\"verificationClass\":\"TEST\",\"verificationPlan\":[\"Run the targeted test suite and require a zero exit code.\"],\"expectedArtifacts\":[\"Targeted test output\"]}.",
    "When the Requirement specifies a verificationClass from the allowed list, preserve it exactly. Preserve verificationPlan, expectedArtifacts, exact output text, hashes, counts, and ordering exactly in the applicable step when specified by the Requirement.",
