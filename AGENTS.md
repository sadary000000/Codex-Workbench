...
15. When the user explicitly asks to run a deferred/backlog test, select its entry from the **frozen** `DEFERRED_TESTS.json`. Require `status=ready` and a 40-character `executionTarget.commit`. Create a separate detached worktree at that exact commit. If the entry declares `protocol.source=execution-target`, read its Runbook, agent plan, cases, result schema, and runner from that exact execution-target worktree, snapshot those files before execution, and never substitute newer files from the moving branch or bootstrap checkout.
16. A deferred `executionTarget.branch` is a locator, not a current-gate promise. If that branch later advances, the exact deferred commit may still be run as historical evidence; forward validation of newer code requires a versioned rebind to a new exact commit.
